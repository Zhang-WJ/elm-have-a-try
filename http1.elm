...
update msg model =
    case msg of 
        ...
        NewGif (Ok newUrl) ->
            ({model | gifUrl = newUrl, error = "OK" }, Cmd.none)
        NewGif (Err err) ->
            ({model | gifUrl = "", error = toString err }, Cmd.none)
...
view model =
    div []
    [ select [] [ option [] [ text model.topic ], option [] [ text "pig" ] ] 
    , img [ src model.gifUrl ] []
    , button [ onClick MorePlease ] [ text "More Pleased" ]
